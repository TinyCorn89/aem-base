...
@SlingServlet(paths = { "/bin/services/ads-list" }, methods = { "GET","POST" }, metatype = true)
@Properties({ 
			...
			if (result != null) {
				for (Node node : JcrUtils.getNodes(result)) {
					...
					try {
						if(StringUtils.equals(action,"advertisers") ){
							String name1 ="";
							if(node.hasProperty("name1")){
								name1 =node.getProperty("name1").getString();
							}
							String province ="";
							if(node.hasProperty("province")){
								province = node.getProperty("province").getString();
							}
							String address ="";
							if(node.hasProperty("address")){
								address = node.getProperty("address").getString();

							}
							String city ="";
							if(node.hasProperty("city")){
								city = node.getProperty("city").getString();
							}
						...
						array.put(curObj);
						System.out.println("json array is==="+array.toString());
					} catch (Exception e) {
