...
function getPermissions($) {
    var appsServiceReq = null;
        appsServiceReq = jQuery.ajax( {
            url : "/bin/services/appsConfigServlet?action=getPermissions",
            type : "POST",
            async : false,
            dataType : "json"
        });
        ...
        .extend(
                CQ.Ext.Viewport,
                {
                    ...
                    constructor : function(config) {
                        ...
                        var store = new CQ.Ext.data.Store(
                                {
                                    proxy : new CQ.Ext.data.HttpProxy(
                                            {
                                                url : "/bin/services/appsConfigServlet?action=getall"
                                            }),
                                    autoLoad : true,
                                                        ...
                                                        if (btnId == 'yes') {
                                                            ...
                                                            if (selections) {
                                                                ...
                                                                var publishReq = null;
                                                                            publishReq = jQuery
                                                                                    .ajax( {
                                                                                        url : "/bin/services/appsConfigServlet",
                                                                                        type : "POST",
                                                                                        async : false,
                        ...
                        this.newDialog.form.url = '/bin/services/appsConfigServlet';
...
CQ.Ext.form.VTypes["appsConfigDuplicateCheck"] = function(value, f) {
    if (!editFlag) {
        var nodePath = "";
        if (value) {
            var path = "/bin/services/appsConfigServlet";
